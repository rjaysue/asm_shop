...
CHECKOUT_UI   DB ' ----------------------------------------------------------------------------  ',10,13
              DB '|                    THANK YOU FOR YOUR PURCHASE!                            | ',10,13
              DB '|                                                                            | ',10,13
              DB '|                                                                            | ',10,13
              DB '|                                                                            | ',10,13
              DB '|____________________________________________________________________________|$',10,13  
...
NEW_USER_DISC DB 0 ; WILL STORE 7 IF NEW USER 
...
;----------------User authentication code------------------------   
    ...
    ;------- NEW USER GIVEN DISCOUNT---------------
    
    lea di, NEW_USER_DISC
    add [di],7    
...
MASK_LEB:
   ...
   SUB AL,48
   ADD CL, AL                  ;STORE NUMBER OF PRODUCTS IN CL
...
CHECKOUT:
   ...
   ;IF NO, PROGRAM WILL CONTINUE 
    ...
    LEA SI, NEW_USER_DISC                    ;NEW USER DISC
    XOR BX,BX                                ;CLEAR BX
    MOV BL,[SI]                              ;LOAD DISC GIVEN INTO BX
    SUB AX, BX                               ;
    
    CALL buy_more               ;BUY MORE DISC, MAKE A DISCOUNT OF HOW MANY SUBJECTS ADDED (FEATURE) 
...
display_current_price proc 
    XCHG BX,AX
    LEA DX,R                ;PRINT CURRENT PRICE 
    MOV AH,9
    INT 21H
    XCHG BX,AX
    ret
display_current_price endp
